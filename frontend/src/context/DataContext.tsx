...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const [careCircleMembers] = useState<CareCircleMember[]>(staticMock.careCircleMembers);
  ...
  const fetchData = async () => {
    try {
      ...
      if (data.medications) {
        parsedMeds = data.medications.map((dbMed: DBMedication): Medication => {
      ...
      if (data.orders) {
        ...
        const mappedOrders = data.orders.map((dbOrder: DBOrder): PharmacyOrder => {
          ...
          let status: PharmacyOrder['status'] = 'placed';
          ...
          const trackingSteps: PharmacyOrder['trackingSteps'] = isPickup
            ? [
          ...
          return {
            ...
            orderDate,
            expectedDate,
            deliveryType: dbOrder.deliveryType.toLowerCase() as PharmacyOrder['deliveryType'],
            cost: dbOrder.cost,
      ...
      if (data.priorAuths && data.medications) {
        const mappedAuths = data.priorAuths.map((dbAuth: DBPriorAuth): PriorAuth => {
          const status = dbAuth.status.toLowerCase();
          const med = data.medications.find((m: DBMedication) => m.id === dbAuth.medicationId);
          ...
          return {
            ...
            planName: 'PPO Gold',
            status: (status === 'submitted' ? 'under-review' : status) as PriorAuth['status'],
            statusLabel,
      ...
      if (data.alerts) {
        const mappedAlerts = data.alerts.map((dbAlert: DBAlert): CaregiverUpdate => {
          return {
            id: dbAlert.id,
            ...
            actionType: dbAlert.actionType.toLowerCase() as CaregiverUpdate['actionType'],
