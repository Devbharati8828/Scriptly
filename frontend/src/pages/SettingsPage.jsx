...
// A simple in-memory store so settings persist within the session
const sessionStore = {
  profile: null,
  insurance: null,
  notifications: null,
...
export default function SettingsPage() {
  ...
  // Profile State — initialize from sessionStore if it exists (survives tab navigation)
  const [profile, setProfile] = useState(() => sessionStore.profile || {
    name: currentUser?.name || 'John Doe',
    email: currentUser?.email || 'john@gmail.com',
    primaryDoctor: currentUser?.primaryDoctor || 'Dr. Patel',
    primaryPharmacy: currentUser?.primaryPharmacy || 'Apollo Pharmacy — MG Road',
  });

  // Insurance State
  const [insurance, setInsurance] = useState(() => sessionStore.insurance || {
    provider: currentUser?.insuranceProvider || 'Star Health Insurance',
    plan: currentUser?.insurancePlan || 'Family Floater Gold',
    memberId: currentUser?.memberId || 'STR-88421930',
  });

  // Notifications State
  const [notifications, setNotifications] = useState(() => sessionStore.notifications || {
  ...
  // App Settings State
  const [appSettings, setAppSettings] = useState(() => sessionStore.appSettings || {
    autoRefill: true,
  ...
  // Keep sessionStore in sync whenever state changes
  useEffect(() => { sessionStore.profile = profile; }, [profile]);
  useEffect(() => { sessionStore.insurance = insurance; }, [insurance]);
  useEffect(() => { sessionStore.notifications = notifications; }, [notifications]);
  useEffect(() => { sessionStore.appSettings = appSettings; }, [appSettings]);

  // Re-initialize from currentUser if it loads after mount
  useEffect(() => {
    if (currentUser && !sessionStore.profile) {
      setProfile({
        name: currentUser.name || 'John Doe',
        email: currentUser.email || 'john@gmail.com',
        primaryDoctor: currentUser.primaryDoctor || 'Dr. Patel',
  ...
  const handleSave = (section) => {
    // Settings are already persisted in sessionStore via useEffect
  ...
  const handleSaveSecurity = async () => {
    ...
    try {
      const token = localStorage.getItem('scripty_token');
    ...
    <div className="p-8 pb-16 max-w-5xl mx-auto w-full">
      ...
      <Tabs defaultValue="profile" orientation="vertical" className="w-full">
        <div className="flex flex-col md:flex-row gap-8 items-start">
          {/* Navigation Sidebar */}
          <TabsList className="flex flex-col bg-white border border-slate-200 p-1.5 rounded-xl w-full md:w-56 shrink-0 h-auto group-data-vertical/tabs:h-auto">
            <TabsTrigger
              value="profile"
              className="w-full justify-start gap-3 py-3 px-4 text-left font-semibold h-auto data-active:bg-blue-600 data-active:text-white rounded-lg"
            >
