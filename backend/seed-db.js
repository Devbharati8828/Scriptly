...
async function main() {
  ...
  try {
    ...
    await pool.query(`
      ...
      VALUES 
        (?, 'John', 'john@example.com', 'password', 'PATIENT', NOW(), NOW()),
        (?, 'Anna', 'anna@example.com', 'password', 'CAREGIVER', NOW(), NOW()),
        (?, 'Dr. Patel', 'dr.patel@clinic.com', 'password', 'CAREGIVER', NOW(), NOW())
    `, [JOHN_ID, ANNA_ID, DRPATEL_ID]);
    ...
    await pool.query(`
      ...
      VALUES 
        (?, ?, 'Atorsattatin', 'Lipitor', '20mg', 'Once daily', 2, 30, 'LOW_SUPPLY', NULL, ?, NOW(), NOW()),
        (?, ?, 'Metformin', 'Glucophage', '1000mg', 'Twice daily', 25, 60, 'ACTIVE', NULL, ?, NOW(), NOW()),
        (?, ?, 'Ozempic', 'Semaglutide', '0.5mg', 'Once weekly', 4, 4, 'ACTIVE', NULL, NULL, NOW(), NOW()),
        (?, ?, 'Advair Inhaler', 'Fluticasone/Salmeterol', '250/50mcg', 'Twice daily', 1, 1, 'ACTIVE', NULL, NULL, NOW(), NOW()),
        (?, ?, 'Zoloft', 'Sertraline', '50mg', 'Once daily', 8, 30, 'PENDING_REFILL', NULL, ?, NOW(), NOW())
    `, [
    ...
    await pool.query(`
      ...
      VALUES (?, ?, ?, 'DOSE', 'marked "Evening Dose Administered"', NOW())
    `, [crypto.randomUUID(), JOHN_ID, ANNA_ID]);
